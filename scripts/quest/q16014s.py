...
if not sm.hasQuest(16013) and not sm.hasQuestCompleted(16013):
	...
	if sm.getUnionLevel() < 500 or sm.getUnionCharacterCount() < 3 or sm.getUnionRank() == 0:
		...
		nSel = sm.sendSay("It's a fine day to bludgeon some dragons!\r\nAre you here about your #bLegion#k?\r\n\r\n"
						  ...
						  "#L4# #b<Collect Raid Coins>#k#l\r\n"
						  "#L5# #b<Exchange inventory legion coins to new coins>#k#l")
		if nSel == 0:
			...
			if coins > 0:
				sm.sendSayOkay("You already collected #b" + str(coins) + " #k of #b#i4310229:##t4310229##k? "
																		 "That's impressive!"
																		 "\r\n\r\n#bThis raid's Coin Total#k#e: " + str(coins) + "#n")
				sm.addUnionCoin(coins, True)
			else:
				sm.sendSayOkay("Uh, it doesn't look like you've earned any Legion Coins. If you're having a tough time with raiding, just come back later. Your Legion members will continue the raid even after you've left.")
		elif nSel == 5:
				ownedQuantity = sm.getQuantityOfItem(4310229)
				amount = sm.sendAskNumber("How many inventory coins do you want to trade in for new ones? \r\n You currently have #b" + str(ownedQuantity)+ "#k Inventory Legion coins", 1, 0, sm.getQuantityOfItem(4310229))
